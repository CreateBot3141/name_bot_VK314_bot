...
def start_prog (user_id,namebot,message_in,status,message_id,name_file_picture,telefon_nome):
    ...
    if status == 'Ввод ссылки ВК': 
        ...
        for line in response:
            user_id_VK  = line['id']
            print ('[+] user_id_VK:',user_id_VK)


            try:
                crop_photo = line['crop_photo']
                #print ('[+] line:',crop_photo)
                ...
                for size in sizes:
                    nm = nm + 1
                    #print ('[+] size:',size['url'])
                    name_file_save = '/home/izofen/Studiya/FL/' + str(user_id_VK)+'_'+str(nm)+'.jpg'
                    save_picture_file(size['url'],name_file_save)
                    print ('[name_file_save]',name_file_save)
                    if nm ==1:
                        iz_telegram.send_photo (user_id,namebot,name_file_save)
                        #print ('========',name_file_save)
            except Exception as e:
                message_out,menu,answer = iz_telegram.send_message (user_id,namebot,"Ссылка не соответствует стандарту",'S',0)
